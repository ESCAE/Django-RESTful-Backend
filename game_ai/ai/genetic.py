...
class Network(Neural):
    ...
    def _import(self, data):
        """Import a previous network for creation."""
...
class Individual(object):
    """Represent an individual neural net."""
    ...
    def evaluate_versus_greedy_bot(self):
        """Evaluate till game completion with greedy bot."""
        game = Game()
        a = self
        b = greedy_bot
        while True:
    ...
    def ind_import(self, data):
        """Import based off an individual."""
        tag = data['tag']
        net = Network([1, 1])
...
class Generation(object):
    ...
    def run_versus_every_possibility(self):
        """Recursive run against all possibilites."""
        for individual in self.individuals:
            individual.evaluate_vs_every_possibility()

    def run_versus_self(self):
        """Have the net play against it self."""
    ...
    def run_versus_greedy_bot(self):
        """Have the net play against greedy bot."""
    ...
    def order(self):
        """Order individuals by score."""
        for i in range((len(self.individuals) - 1), 0, -1):
            floor()

    def select(self, pool):
        """Select a random individual, leans to strongest."""
        x = random.random()
        x = x * x
    ...
    def next_under_greedybot(self, mutation_rate=0.05, tag=-1):
        """Make the new generation for training under greedy bot."""
        for individual in self.individuals:
    ...
    def new_random(self, size=100, sizes=[18, 27, 9, 1], tag=0, imported=[]):
        """Make a geneartion with random indivduals."""
...
path_to_pickle = path.realpath(__file__).replace('genetic.py', 'defensebot')
...
def move(board, move):
    ...
    #     neural = fp.read()
    neural = json.loads(open(path_to_pickle).read())
    ...
    neural = neural['individuals'][0]['net']
    # new_gen = Generation([])
    new_net = Network()
    # bot = new_gen._import(neural)
